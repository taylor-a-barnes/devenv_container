...
switch ($choice) {
    "1" {
        ...
        Write-Host ""
        docker run --rm -it --gpus all -v ${currentDir}:/work ${image} bash /.nvim/entrypoint.sh
    }
    "2" {
        ...
        Write-Host ""
        docker run --rm -it --gpus all -v ${currentDir}:/work -p 127.0.0.1:${port}:8080 ${image} bash /.code-server/entrypoint.sh
    }
    "3" {
        Write-Host "Entering an interactive terminal session"
        Write-Host ""
        docker run --rm -it --gpus all -v ${currentDir}:/work ${image}
    }
    Default {
