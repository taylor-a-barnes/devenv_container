...
case $choice in
  1)
    echo "Opening Neovim"
    echo ""
    docker run --rm -it --gpus all -v $(pwd):/work ${image} bash /.nvim/entrypoint.sh
    ;;
  2)
    ...
    echo ""
    docker run --rm -it --gpus all -v $(pwd):/work -p 127.0.0.1:${port}:8080 ${image} bash /.code-server/entrypoint.sh
    ;;
  3)
