# Get the image name
param(
    [string]$image="docker.io/taylorabarnes/devenv",
    [Int32]$port=56610
)
...
switch ($choice) {
    "1" {
        ...
        Write-Host ""
        podman run --rm -it -v ${currentDir}:/work ${image} bash /.nvim/entrypoint.sh
    }
    "2" {
        # Check if any container is already using the port
        $containerId = ""
        $containerId = podman ps --filter "publish=$port" -q | Select-Object -First 1
        if ($containerId) {
        ...
        Write-Host ""
        podman run --rm -it -v ${currentDir}:/work -p 127.0.0.1:${port}:8080 ${image} bash /.code-server/entrypoint.sh
    }
    "3" {
        Write-Host "Entering an interactive terminal session"
        Write-Host ""
        podman run --rm -it -v ${currentDir}:/work ${image}
    }
    Default {
