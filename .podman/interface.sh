...
case $choice in
  1)
    echo "Opening Neovim"
    echo ""
    podman run --rm -it -v $(pwd):/work ${image} bash /.nvim/entrypoint.sh
    ;;
  2)
    ...
    echo ""
    podman run --rm -it -v $(pwd):/work -p 127.0.0.1:${port}:8080 ${image} bash /.code-server/entrypoint.sh
    ;;
  3)
    echo "Entering an interactive terminal session"
    echo ""
    podman run --rm -it -v $(pwd):/work ${image}
    ;;
  *)
    echo "Invalid option."
    exit 1
